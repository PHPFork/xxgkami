...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['generate_card']) && isset($_POST['action']) && $_POST['action'] == 'add') {
        ...
        $stmt = $conn->prepare("INSERT INTO cards (card_key, encrypted_key, duration, allow_reverify, card_type, total_count, remaining_count,good_id) VALUES (?, ?, ?, ?, ?, ?, ?,?)");
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $stmt->execute([$key['original'], $key['encrypted'], $duration, $allow_reverify, $card_type, $total_count, $remaining_count, $goodId]);
        }
            ...
            <form method="POST" class="form-group" style="padding: 20px;">
                ...
                <div class="generate-form">
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            <label><i class="fas fa-credit-card"></i> 商品：</label>
                            <select name="good_id" id="good_id" class="form-control">
                                <option value="0">请选择</option>
                                <option value="1">蘑菇箱</option>
                                <option value="2">超级菇菇</option>
                            </select>
                        </div>
                ...
                <table>
                    ...
                    <?php foreach ($cards as $card): ?>
                        <tr>
                            ...
                            <td>
                                <input type="text" value="<?php echo $card['card_key']; ?>" readonly
                                       style="width: 180px;">
                            ...
                            <td>
                                <?php if ($card['status'] == 0): // 未使用 ?>
                                    <button type="button" class="btn btn-danger btn-sm"
                                            onclick="deleteUnusedCard(<?php echo $card['id']; ?>)"
                                            title="删除未使用卡密">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                <?php else: // 已使用或已停用 ?>
                                    <?php if ($card['status'] == 1): // 正常状态显示停用按钮 ?>
                                        <button type="button" class="btn btn-warning btn-sm">收货信息</button>
                                        <button type="button" class="btn btn-warning btn-sm"
                                                onclick="toggleCardStatus(<?php echo $card['id']; ?>, 'disable')"
                                                title="停用卡密">
                                            <i class="fas fa-ban"></i>
                                        </button>
                                    <?php elseif ($card['status'] == 2): // 停用状态显示启用按钮 ?>
                                        <button type="button" class="btn btn-success btn-sm"
                                                onclick="toggleCardStatus(<?php echo $card['id']; ?>, 'enable')"
                                                title="启用卡密">
                                            <i class="fas fa-check"></i>
                                        </button>
                                    <?php endif; ?>
                                    <button type="button" class="btn btn-info btn-sm"
                                            onclick="promptExpireTime(<?php echo $card['id']; ?>, '<?php echo $card['expire_time']; ?>')"
                                            title="修改时间">
                                        <i class="fas fa-clock"></i>
                                    </button>
                                    <?php if ($card['card_type'] == 'count'): // 如果是次数卡密，显示修改次数按钮 ?>
                                        <button type="button" class="btn btn-info btn-sm"
                                                onclick="promptRemainingCount(<?php echo $card['id']; ?>, <?php echo $card['remaining_count']; ?>)"
                                                title="修改次数">
                                    ...
                                    <?php if (!empty($card['device_id'])): // 如果已绑定设备，显示解绑按钮 ?>
                                        <button type="button" class="btn btn-secondary btn-sm"
                                                onclick="unbindDevice(<?php echo $card['id']; ?>)" title="解绑设备">
                                            <i class="fas fa-unlink"></i>
                                        </button>
                                    <?php endif; ?>
                                    <button type="button" class="btn btn-danger btn-sm"
                                            onclick="deleteUsedCard(<?php echo $card['id']; ?>)" title="删除卡密">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                <?php endif; ?>
                            </td>
